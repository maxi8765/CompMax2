...
/**
 * Format salary input with commas as thousands separators
...
function buildShareUrl(baseUrl, data) {
    ...
    params.append('eemail', data.employeeEmail); // Add employee email to URL parameters
 ...
 */
/**
 * Send offer email to employee with employer CC'd
 */
/**
 * Send offer email to employee with employer CC'd
 */
function sendOfferEmail() {
    ...
    // Prepare template parameters for EmailJS with CC
    const templateParams = {
        to_name: details.employeeName,
        to_email: details.employeeEmail,
        cc: details.employerEmail, // Add employer as CC
        subject: subject,
        message: body,
    ...
    emailjs.send(CONFIG.emailjs.serviceId, CONFIG.emailjs.templateId, templateParams)
        .then(function(response) {
            ...
            Elements['sending-status'].textContent = 'Email sent successfully to both employee and employer!';
            Elements['sending-status'].style.color = 'var(--success)';
            Elements['send-email-button'].disabled = true;
            Elements['send-email-button'].textContent = 'Email Sent ✓';
        })
        .catch(function(error) {
            console.error('Email sending failed:', error);
            Elements['sending-status'].textContent = 'Failed to send email. Please use the manual method below.';
            Elements['sending-status'].style.color = 'var(--danger)';
...
function sendAcceptanceEmail() {
    ...
    // Get employee email from URL if available
    const urlParams = new URLSearchParams(window.location.search);
    const employeeEmail = urlParams.get('eemail') || '';
    
    // Prepare template parameters for EmailJS
    const templateParams = {
        to_name: details.senderName,
        to_email: details.employerEmail,
        cc: employeeEmail, // Add employee as CC if available
        subject: details.subject,
        time: new Date().toLocaleString(),
        message: details.body,
        from_name: offereeNameSending,
        reply_to: employeeEmail || '' // Use employee email as reply-to if available
    };
    ...
    emailjs.send(CONFIG.emailjs.serviceId, CONFIG.emailjs.templateId, templateParams)
        .then(function(response) {
            console.log('Email sent successfully:', response.status, response.text);
            Elements['acceptance-sending-status'].textContent = 'Acceptance email sent successfully!';
            Elements['acceptance-sending-status'].style.color = 'var(--success)';
            Elements['send-acceptance-email-button'].disabled = true;
            Elements['send-acceptance-email-button'].textContent = 'Email Sent ✓';
        })
        .catch(function(error) {
            console.error('Email sending failed:', error);
            Elements['acceptance-sending-status'].textContent = 'Failed to send email. Please use the manual method below.';
            Elements['acceptance-sending-status'].style.color = 'var(--danger)';
            showError('Could not send email automatically. Please use the manual method instead.');
        });
}
